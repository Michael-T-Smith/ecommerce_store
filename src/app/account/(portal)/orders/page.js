...
const STATUS_META = {
  ...
  out_for_delivery: { label: "Out for Delivery", color: "#C08FA3" },
  delivered       : { label: "Delivered",        color: "#22C55E" },
  cancelled       : { label: "Cancelled",        color: "#EF4444" },
};

function OrderStatusBadge({ status }) {
  const meta = STATUS_META[status] ?? { label: status, color: "#8C8288" };
  return (
    <span
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="font-sans text-brand-smoke text-[13px]">
          {orders.length} {orders.length === 1 ? "order" : "orders"} placed with BityBird Co
        </p>
      </div>
        ...
        <div className="flex flex-col gap-4">
          {orders.map((order) => (
            <Link
              ...
              style={{ borderLeft: `4px solid ${STATUS_META[order.status]?.color ?? "#8C8288"}` }}
            >
