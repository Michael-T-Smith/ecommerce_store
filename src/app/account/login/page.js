...
const handleRegister = async () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden" style={{ background: C.cream }}>
      ...
      <div className="w-full max-w-[460px] relative z-10">
        <div className="text-center mb-8">
          <Link href="/" className="no-underline inline-block">
            <div className="font-serif font-black text-brand-black text-[28px] tracking-[-1px] leading-none">BityBird Co</div>
        ...
        <div className="bg-white border-[3px] border-brand-black shadow-retro-lg overflow-hidden">
          <div className="h-[5px] w-full" style={{
            background: `repeating-linear-gradient(90deg, ${C.blush} 0, ${C.blush} 40px, ${C.gold} 40px, ${C.gold} 50px, ${C.blush} 50px, ${C.blush} 90px, ${C.black} 90px, ${C.black} 94px)`,
          }} />
          ...
          <div className="flex border-b border-gray-200">
            {[{ key: "signin", label: "Sign In" }, { key: "register", label: "Create Account" }].map((t) => (
              <button key={t.key} onClick={() => { setTab(t.key); setError(null); }}
                ...
                style={{ color: tab === t.key ? C.blush : "#8C8288", borderBottom: tab === t.key ? `3px solid ${C.blush}` : "3px solid transparent", marginBottom: "-1px" }}>
                {t.label}
              </button>
            ))}
          </div>
