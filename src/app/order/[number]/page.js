...
export default function OrderConfirmationPage() {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("bitybird_pending_order");
      if (raw) {
        const parsed = JSON.parse(raw);
        setOrder(parsed);
        sessionStorage.removeItem("bitybird_pending_order");
      }
    } catch { /* non-fatal */ }
    setReady(true);
  }, []);
              ...
              <div className="mb-6 border-[3px] border-brand-orange bg-white overflow-hidden shadow-retro-sm">
                <div className="bg-brand-orange px-6 py-3 flex items-center gap-2">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                    stroke={C.cream} strokeWidth="2.5" strokeLinecap="round">
              ...
              <div className="bg-white border-[3px] border-brand-black shadow-retro-sm overflow-hidden">
                <div className="bg-brand-bark px-6 py-4 flex items-center gap-3">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                    stroke={C.cream} strokeWidth="2" strokeLinecap="round">
                      ...
                      <div key={i} className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          {item.customizationText && (
                            <div className="font-sans text-[10px] text-brand-smoke/70 italic mt-0.5">
                              {capitalise(item.customizationType || "personalized")}: &ldquo;{item.customizationText}&rdquo;
                            </div>
                          )}
                        </div>
                ...
                {[
                  {
                    emoji: "📞",
                    title: "We'll confirm",
                    body : "Our team will reach out to confirm your order and arrange payment.",
                  },
                  {
                    emoji: "📦",
                    title: "We'll prepare",
                    body : "Your item is carefully packed and prepared on the day of dispatch.",
                  },
                  {
                    emoji: "🚗",
                    title: "We'll deliver",
