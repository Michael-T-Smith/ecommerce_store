...
export default function OrdersPage() {
  ...
  useEffect(() => { setPage(1); }, [filterStatus, search, sortCol, sortDir, hideArchived]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex gap-1 overflow-x-auto pb-1">
        {[{ key: "all", label: "All", color: C.darkGrey }, ...ORDER_STATUSES].map((s) => (
                  ...
                  <tr key={order.id} onClick={() => setSelected(order)}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {order.items?.some((i) => i.customizationText) && (
                          <span
                            title="Contains personalized items"
                            className="font-sans font-extrabold text-[8px] tracking-[1px] uppercase px-1.5 py-0.5 border border-brand-orange/40 text-brand-orange bg-brand-orange/5"
                          >
                            Custom
                          </span>
                        )}
                      ...
                      <div className="flex gap-1.5 items-center">
                        ...
                        {canUpdate && nextMeta && (
                          <button onClick={() => handleAdvance(order.id, nextStatus)}
                            ...
                            onMouseEnter={(e) => { e.currentTarget.style.background = nextMeta.color; e.currentTarget.style.color = "#FAF8F4"; }}
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
              className="font-sans font-extrabold text-[10px] tracking-[1px] uppercase px-4 py-2 border-2 border-gray-200 text-brand-smoke bg-white cursor-pointer hover:border-brand-black hover:text-brand-black transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
              ← Prev
            </button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
              <button key={n} onClick={() => setPage(n)}
                className={`w-9 h-9 font-sans font-extrabold text-[11px] border-2 cursor-pointer transition-all ${
                  n === page
                    ? "bg-brand-orange text-brand-cream border-brand-orange"
                    : "bg-white text-brand-smoke border-gray-200 hover:border-brand-black"
                }`}>
                {n}
              </button>
            ))}
