...
function remapItem(row) {
  return {
    ...
    customizationType : row.customization_type  ?? "engraved",
...
export default function InventoryPage() {
  ...
  const handleSave = async (formData) => {
    try {
      if (modalMode === "add") {
        // Pull out local preview images (File objects attached before item existed)
        const pendingImages = (formData.images || []).filter((img) => img.file);
        const res     = await createInventoryItem(formData);
        const newItem = res.data;

        // Upload any images staged during creation now that we have an ID
        for (const img of pendingImages) {
          const fd = new FormData();
          fd.append("file", img.file);
          await fetch(`/api/inventory/${newItem.id}/images`, { method: "POST", body: fd });
        }

        setItems((prev) => [...prev, remapItem(newItem)]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard label="Total" value={String(items.length)} sub="products" accent={C.blush}
        ...
        <StatCard label="Running Low" value={String(lowCount)} sub="below threshold" accent={C.gold}
