...
const STATUS_COLORS = {
  ...
  confirmed       : "#3B82F6",
  preparing       : "#8B5CF6",
  out_for_delivery: "#C08FA3",
  delivered       : "#22C55E",
  cancelled       : "#EF4444",
};

function StatBox({ label, value, sub, accent = C.blush }) {
  return (
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatBox label="Total Revenue" value={`$${fmt(stats.revenue)}`}   sub="non-cancelled orders" accent={C.blush} />
        <StatBox label="Orders"        value={String(stats.total)}         sub={`${stats.delivered} delivered`} accent={C.darkGrey} />
        <StatBox label="Avg. Order"    value={`$${fmt(stats.avgOrder)}`}  sub="per transaction" accent={C.gold} />
              ...
              <div key={d.date} className="flex-1 flex flex-col items-center justify-end gap-1 group relative">
                <div className="w-full transition-all duration-300 cursor-default"
                  style={{
                    ...
                    background: d.count > 0 ? C.blush : "#E5E7EB",
                  }} />
            ...
            {Object.entries(stats.statuses)
              ...
              .map(([status, count]) => (
                <HBar key={status} label={status.replace(/_/g, " ")} value={count}
                  max={stats.total} color={STATUS_COLORS[status] ?? "#8C8288"} />
              ))}
          ...
          <div className="flex flex-col gap-3">
            {stats.topProducts.map(([name, { units, revenue }]) => (
              <HBar key={name} label={name} value={units} max={stats.maxUnits}
                color={C.blush} suffix=" units" sub={`$${fmtShort(revenue)}`} />
            ))}
        ...
        <div className="bg-white border border-gray-200 p-6">
          ...
          {totalFulfillment === 0 ? (
            ...
            <div className="flex flex-col gap-6">
              {[
                { key: "delivery", label: "Delivery", color: C.blush },
                { key: "pickup",   label: "Pickup",   color: C.darkGrey   },
              ].map(({ key, label, color }) => {
