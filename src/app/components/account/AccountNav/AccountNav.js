      ...
      {NAV_ITEMS.map((item) => {
        ...
        return (
          <Link
            ...
            style={{
              borderColor: isActive ? B.black : "transparent",
              background : isActive ? "white" : "transparent",
              color      : isActive ? B.orange : B.smoke,
              boxShadow  : isActive ? `3px 3px 0 ${B.black}` : "none",
            }}
          >
            <span style={{ color: isActive ? B.orange : B.smoke }}>{item.icon}</span>
            {item.label}
          </Link>
        );
      })}
